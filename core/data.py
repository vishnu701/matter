...
def create_dataset(meta_df,
                   ...
                   target='data/records/macho/train',
                   n_jobs=None,
    ...
    for cls_name, frame in tqdm(cls_groups, total=len(cls_groups)):
        dest = os.path.join(target, cls_name)
...
def mask_sample(x, y , i, msk_prob, rnd_prob, same_prob, max_obs):
    ...
    input_dict['mask_out'] = 1.-mask_out
    input_dict['mask_in']  = 1.-mask_in
...
def format_pretraining(input_dict):
    x = {
    'input':input_dict['input'],
    'times':input_dict['times'],
    'mask_in':input_dict['mask_in']
    }
    y_target = tf.concat([input_dict['output'], input_dict['mask_out']], 1)
    return x, y_target


def load_records(source, batch_size, max_obs=100, msk_frac=0.,
                        rnd_frac=0., same_frac=0., sampling=False,
    ...
    dataset = tf.data.TFRecordDataset(rec_paths).cache()
